...
def extract_feedback_categories(text: str):
    ...
    for line in text.split('\n'):
        ...
        if any(word in line_lower for word in ['love', 'like', 'amazing', 'great', 'awesome', 'enjoyed']):
            likes.append(line)
        elif any(word in line_lower for word in ['hate', 'bad', 'worst', 'boring', 'dislike', 'terrible', 'too long', 'annoying']):
            dislikes.append(line)
        elif any(word in line_lower for word in ['should', 'need', 'please', 'can you', 'add', 'include', 'want', 'make a']):
            demands.append(line)
